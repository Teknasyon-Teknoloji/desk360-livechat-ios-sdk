    ...
    internal func onDeviceOrientation(_ fullScreen: Bool, orientation: UIInterfaceOrientation) {
        ...
        if orientation == statusBarOrientation {
            if orientation == .landscapeLeft || orientation == .landscapeLeft {
				...
				setSize(.init(width: superview!.bounds.width, height: superview!.bounds.height))
            }
        } else {
            if orientation == .landscapeLeft || orientation == .landscapeRight {
				...
				centerInSuperview()
				setSize(.init(width: frame.width, height: frame.height))
...
extension VGPlayerView {
    ...
    internal func configurationTopView() {
        ...
        let closeImage = VGPlayerUtils.imageResource("back_white")
    ...
    internal func setupViewAutoLayout() {
		...
		replayButton.centerInSuperview()
		replayButton.setSize(.init(width: 30, height: 30))

		topView.anchor(top: topAnchor, leading: leadingAnchor, bottom: nil, trailing: trailingAnchor, size: .init(width: 0, height: 64))
		
		closeButton.anchor(top: topView.topAnchor, leading: topView.leadingAnchor, bottom: nil, trailing: nil, padding: .init(v: 28, h: 10), size: .init(width: 30, height: 30))
		titleLabel.anchor(top: nil, leading: closeButton.trailingAnchor, bottom: nil, trailing: nil, padding: .init(v: 0, h: 20))
		titleLabel.centerYTo(closeButton.centerYAnchor)
        
		bottomView.anchor(top: nil, leading: leadingAnchor, bottom: safeAreaLayoutGuide.bottomAnchor, trailing: trailingAnchor, padding: .zero, size: .init(width: 0, height: 52))
        
		fullscreenButton.setSize(.init(width: 30, height: 30))
		playButtion.setSize(.init(width: 25, height: 25))
		let stack: UIView = .hStack(alignment: .fill, distribution: .fill, spacing: 10, margins: nil, [playButtion, timeSlider, timeLabel, fullscreenButton])
		
		bottomView.addSubview(stack)
		stack.fillSuperview(padding: .init(top: 8, left: 10, bottom: 8, right: 10))
		loadingIndicator.centerInSuperview(size: .init(width: 30, height: 30))
    }
}
